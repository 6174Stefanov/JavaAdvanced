...
public class E8WrongMeasurments {
    public static void main(String[] args) {
        ...
        int wrongElement = matrix[indexes[0]][indexes[1]];

        List<int[]> correctValues = new ArrayList<>();
        for (int r = 0; r < matrix.length; r++) {
            for (int c = 0; c < matrix[r].length; c++) {
                if (matrix[r][c] == wrongElement) {
                    int correctValue = getNearbySum(matrix,r,c, wrongElement);
    ...
    private static int getNearbySum(int[][] matrix, int r, int c, int wrongElement) {
        int sum = 0;

        if (isInBounds(matrix, r+1, c) && matrix[r+1][c] != wrongElement) {
            sum += matrix[r+1][c];
        }
        if (isInBounds(matrix, r-1,c) && matrix[r-1][c] != wrongElement) {
            sum += matrix[r-1][c];
        }
        if (isInBounds(matrix, r,c + 1) && matrix[r][c+1] != wrongElement) {
            sum += matrix[r][c + 1];
        }
        if (isInBounds(matrix,r ,c - 1) && matrix[r][c-1] != wrongElement) {
            sum += matrix[r][c - 1];
